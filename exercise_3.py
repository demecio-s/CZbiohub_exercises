# Measure the eccentricity of each nucleus you detected and plot the results as a histogram. 
# Label the histogram such that we can understand the data it shows.
...
with tiff.TiffFile('Assoc_RDEng_test.tif') as images:
    for iter, page in enumerate(images.pages, 1):
        ...
        #creating threshold for region based segmentation similar to: https://scikit-image.org/docs/stable/user_guide/tutorial_segmentation.html#region-based-segmentation
        std_dev = image.std()
        min = image.min()
